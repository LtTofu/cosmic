...
#define VTSEQ(ID) ("\x1b[" #ID "m")
	...
	Text ec_to_text(EcHandle ec_handle)
	{
		...
		char* with_newline = reinterpret_cast<char*>(malloc(buffer_size));				// MSVC code analysis warning C6011: dereferencing NULL pointer `with_newline`
		with_newline[0] = '\n';															// [FIXME] <---	if the malloc() failed, handle that <--- [FIXME]
	#ifdef _WIN32
